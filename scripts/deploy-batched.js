...
const BATCH_SIZE = 20;
const BATCH_DELAY = 300000; // 5 minutes
...
async function contractExists(name) {
...
async function main() {
    console.log(`🚀 Batched deployment to Stacks Mainnet`);
    ...
    for (let i = 0; i < contracts.length; i += BATCH_SIZE) {
        ...
        for (const contract of batch) {
            if (await contractExists(contract)) {
    ...
    console.log(`\n✅ Complete! Deployed: ${deployed}, Skipped: ${skipped}`);
}
