;; blitmap.clar
;; Popular NFT Collection: Blitmap
;; Implementation of SIP-009

(impl-trait .sip-009-nft-trait.nft-trait)

(define-non-fungible-token blitmap uint)
...
(define-data-var base-uri (string-ascii 256) "https://blitmap.com/api/")
...
(define-read-only (get-owner (token-id uint))
    (ok (nft-get-owner? blitmap token-id))
)

(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin
        (asserts! (is-eq tx-sender sender) ERR_NOT_TOKEN_OWNER)
        (nft-transfer? blitmap token-id sender recipient)
    )
)
...
(define-public (mint (recipient principal))
    (let
        ...
        (try! (nft-mint? blitmap token-id recipient))
