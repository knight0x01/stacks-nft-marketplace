;; mfers.clar
;; Popular NFT Collection: mfers
;; Implementation of SIP-009

(impl-trait .sip-009-nft-trait.nft-trait)

(define-non-fungible-token mfers uint)
...
(define-data-var base-uri (string-ascii 256) "https://mfers.api/")
...
(define-read-only (get-owner (token-id uint))
    (ok (nft-get-owner? mfers token-id))
)

(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin
        (asserts! (is-eq tx-sender sender) ERR_NOT_TOKEN_OWNER)
        (nft-transfer? mfers token-id sender recipient)
    )
)
...
(define-public (mint (recipient principal))
    (let
        ...
        (try! (nft-mint? mfers token-id recipient))
